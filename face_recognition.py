    ...
    def face_recog(self):
        def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for (x, y, w, h) in features:
                ...
                if n is not None:  # Check if n is None before trying to iterate over it
                    n = "+".join(map(str, n))  # Convert elements to strings

                    cursor.execute("select Roll_No from student where student_id=" + str(id))
                    r = cursor.fetchone()
                    r = "+".join(map(str, r))  # Convert elements to strings

                    cursor.execute("select student_id from student where student_id=" + str(id))
                    i = cursor.fetchone()
                    i = "+".join(map(str, i))  # Convert elements to strings

                    if confidence > 77:
                        cv2.putText(img, f"student_id:{i}", (x, y - 80), cv2.FONT_HERSHEY_COMPLEX, 0.8, (64, 15, 223),
                                    2)
        ...
        clf = cv2.face.LBPHFaceRecognizer_create() if cv2.__version__.startswith(
